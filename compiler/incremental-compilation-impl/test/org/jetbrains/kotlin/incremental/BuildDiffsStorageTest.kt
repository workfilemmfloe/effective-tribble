...
class BuildDiffsStorageTest {
    ...
    fun testToString() {
        ...
        Assert.assertEquals("BuildDiffsStorage(buildDiffs=[BuildDifference(ts=100, dirtyData=DirtyData(dirtyLookupSymbols=[LookupSymbol(name=foo, scope=bar)], dirtyClassesFqNames=[fizz.Buzz]))])",
                            diffs.toString())
    }
    ...
    fun writeReadMany() {
        ...
        val diffsDeserialized = BuildDiffsStorage.readFromFile(storageFile, reporter = null)
        val expected = generated.sortedBy { it.ts }.takeLast(10).toTypedArray()
