...
data class BuildDiffsStorage(val buildDiffs: List<BuildDifference>) {
    companion object {
            ...
            try {
                ObjectInputStream(file.inputStream().buffered()).use { input ->
                    ...
                    if (version != CURRENT_VERSION) {
                        reporter?.report { "Could not read diff from friend module: version is different $version" }
                        return null
                    }
                ...
                reporter?.report { "Could not read diff from friend module: $e" }
            }
        ...
        private val MAX_DIFFS_ENTRIES: Int = 10
        @set:TestOnly
